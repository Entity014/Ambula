...
void moveBase()
{
    ...
    {
        odrv_left_wheel.setVelocity(twist_msg.angular.x);
        odrv_right_wheel.setVelocity(twist_msg.angular.y);
    }
    else
    {
...
void publishData()
{
    ...
    if (have_wheel && dt > 0.0f)
    {
        ...
        if (ok)
        {
            ...
            // stamp ให้ตรงกับ time_stamp ที่คุณสร้างจาก agent sync
            odom_msg.header.stamp.sec = time_stamp.tv_sec;
            odom_msg.header.stamp.nanosec = time_stamp.tv_nsec;

            // frame_id/child_frame_id ถูก set แล้วใน odom.setFrames()
