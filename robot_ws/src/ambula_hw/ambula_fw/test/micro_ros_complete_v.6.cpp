...
rcl_subscription_t height_subscriber;
...
std_msgs__msg__Float32 height_msg;
...
void publishData()
{
    ...
    struct Map
    {
        ...
        {&odrv_left_hip, &D_left_hip, J_LEFT_HIP, 2.0f * M_PI / 20.0f, 2.0f * M_PI / 20.0f},
        {&odrv_left_knee, &D_left_knee, J_LEFT_KNEE, 2.0f * M_PI / 10.0f, 2.0f * M_PI / 10.0f},
        {&odrv_left_wheel, &D_left_wheel, J_LEFT_WHEEL, 2.0f * M_PI * 0.67, 2.0f * M_PI * 0.67},
        {&odrv_right_hip, &D_right_hip, J_RIGHT_HIP, 2.0f * M_PI / 20.0f, 2.0f * M_PI / 20.0f},
        {&odrv_right_knee, &D_right_knee, J_RIGHT_KNEE, 2.0f * M_PI / 10.0f, 2.0f * M_PI / 10.0f},
        {&odrv_right_wheel, &D_right_wheel, J_RIGHT_WHEEL, 2.0f * M_PI * 0.67, 2.0f * M_PI * 0.67}};
    ...
    for (auto &m : maps)
    {
        ...
        {
            joint_state_msg.position.data[m.pos_idx] = fb.Pos_Estimate * m.pos_scale;
            joint_state_msg.velocity.data[m.pos_idx] = fb.Vel_Estimate * m.vel_scale;

            if (m.pos_idx == J_LEFT_WHEEL)
            {
                wheel_pos_l_rad = joint_state_msg.position.data[m.pos_idx];
                got_left_wheel = true;
            }
            if (m.pos_idx == J_RIGHT_WHEEL)
            {
                wheel_pos_r_rad = joint_state_msg.position.data[m.pos_idx];
...
bool destroyEntities()
{
    ...
    rcl_subscription_fini(&height_subscriber, &node);
